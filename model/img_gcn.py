...
class ImageGCNLayer(nn.Module):
    ...
    def forward(self, g):
        g.apply_nodes(func=self.apply_node)  # 每个节点应用一次传入的方程
    ...
    def message(self, edges):
        z1 = edges.src['att'] * edges.src['h']  # 相邻节点attention * 相邻节点的表示
        z2 = edges.data['att'] * self.rel_fc(edges.data['rel'])  # 边attention * 边表示
        msg = torch.cat([z1, z2], dim=1)
        return {'msg': msg}  # 放到节点的mailbox中
