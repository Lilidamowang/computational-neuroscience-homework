...
class CMGCNnet_RIM_FVQA(nn.Module):
    def __init__(self, config, que_vocabulary, glove, device, n_layers, num_units, hidden_size):
        '''
        :param config: 配置参数
        :param que_vocabulary: 字典 word 2 index
        :param glove: (voc_size,embed_size)
        '''
        super(CMGCNnet_RIM_FVQA, self).__init__()
        ...
        # RIM部分
        self.rim_layer = RIM(device='cuda:0', input_size=900, hidden_size=128, \
                             num_units=num_units, k=2, rnn_cell='LSTM', n_layers=self.n_layers, bidirectional=True)
        self.mlp_rim = nn.Sequential(
            nn.Linear(1280, 1024),
            nn.ReLU(),
            nn.Linear(1024, 512),
            nn.ReLU(),
        ...
        self.rim_cell = RIMCell(self.device, 300, hidden_size, num_units, 4, "LSTM").to(self.device)
        ...
        self.rim_cell_action3 = RIMCell(self.device, 900, 128, 8, 4, "LSTM").to(self.device)
        self.rim_cell_action4 = RIMCell(self.device, 300, 128, 5, 2, "LSTM").to(self.device)

        self.rim_action5 = RIM(device=device, input_size=300, hidden_size=128,
                               num_units=8, k=4, rnn_cell='LSTM', n_layers=5, bidirectional=True)
        self.mlp_action5 = nn.Sequential(
            nn.Linear(1612, 1024),
            nn.ReLU(),
            nn.Linear(1024, 512),
            nn.ReLU(),
        ...
        self.rim_action6 = RIM(device=device, input_size=300, hidden_size=128,
                               num_units=3, k=2, rnn_cell='LSTM', n_layers=5, bidirectional=True)
        self.mlp_action6 = nn.Sequential(
            nn.Linear(896, 512),
            nn.ReLU(),
            nn.Linear(512, 2)
        )

        self.rim_action7 = RIM(device=device, input_size=300, hidden_size=128,
                               num_units=3, k=2, rnn_cell='LSTM', n_layers=5, bidirectional=True)
        self.mlp_action7 = nn.Sequential(
            nn.Linear(384, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
        ...
        self.rim_action8 = RIM(device=device, input_size=300, hidden_size=128,
                               num_units=4, k=2, rnn_cell='LSTM', n_layers=5, bidirectional=True)
        self.ques_down8 = nn.Linear(512, 300)
        self.mlp_action8 = nn.Sequential(
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        self.rim_action11 = RIM(device=device, input_size=300, hidden_size=128,
                                num_units=3, k=2, rnn_cell='LSTM', n_layers=2, bidirectional=True)
        self.mlp_action11 = nn.Sequential(
            nn.Linear(896, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
    ...
    def forward(self, batch):
        ...
        images = batch['features_list']  # [(36,2048)]  一共36个Node 每个Node的embedding_size为2048
        ...
        _, (ques_embed, _) = self.ques_rnn(ques_embed, questions_len_list)  # qes_embed shape=(batch,512)
        ...
        for i in range(batch_size):
            ...
            g.ndata['batch'] = torch.full([36, 1], i).to(
                self.device)  # torch.Size([36, 1]) full with i  表示这个结点属于第i个question
            # add edges
            for s in range(36):
                for d in range(36):
                    g.add_edge(s, d)
        ...
        fact_batch_graph.ndata['hh'] = fact_batch_graph.ndata['h']  # 将起初的fact node embedding保存
        ...
        new_fact_batch_graph.ndata['rim_h'] = self.action_1(new_fact_batch_graph, ques_embed)

        ''' MLP二分类 '''
        new_fact_batch_graph.ndata['h'] = self.mlp_action1(new_fact_batch_graph.ndata['rim_h'])
        return new_fact_batch_graph
    ...
    def action_1(self, new_fact_batch_graph, ques_embed):
        ...
        for i, g in enumerate(g_list):
            ...
            for j in range(0, fac_nod.shape[0]):
                ...
                x_sem[i, j] = sem_mem[j]
                x_fac[i, j] = fac_nod[j]
        x = torch.cat((x_vis, x_sem, x_fac), dim=1).float()
        xs = torch.split(x, 1, 1)
        hs = torch.randn(x.size(0), 8, 100).to(self.device)
        cs = torch.randn(x.size(0), 8, 100).to(self.device)
        for x in xs:  # shape = (16, num_node, 300)
            x = x.to(self.device)
            hs, cs = self.rim_cell_action1(x, hs, cs)
        rim_h = hs.contiguous().view(x.size(0), -1)  # h.view = [16, 800]  一个batch中有16个问题，每个问题对应一个h
