...
class MemoryWrite(nn.Module):
    ...
    def reduce(self, nodes):  # 聚合函数
        ...
        return {'h': new_memory}  # 得到新的节点特征 h
...
class MemoryNetwork(nn.Module):
    ...
    """
        ...
        query_hidden_size :
    """
    def __init__(self, query_input_size, memory_size, que_szie, query_hidden_size, memory_relation_size, memory_hidden_size, mem_read_att_proj, T, device):
    ...
    def forward(self, query_graph, memory_graph, question):
        ...
        mem_his = torch.zeros(num_querys, self.T, 300).to(self.device)
        for t in range(self.T):  # 循环执行t步，更新fact graph的控制信号 h ， 外面取出 h 参数，放入特定位置
            ...
            mem_his[:, t] = query_graph.ndata['h']
        # ndata的shape要match node的num
        query_graph.ndata['a'] = mem_his  # query_graph.ndata['a'].shape = [53, T, 300]
        # 返回的是h，而不是mem
        return query_graph
